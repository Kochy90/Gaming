...
import static integration_test.com.gaming_platform.IntegrationTestConstants.createHigherOrLowerGameCommand;
...
class GameServiceIntTest {
    ...
    @Autowired
    private SinglePlayerSingleBetGameService<HigherOrLowerGame, HigherOrLowerPlayer, HigherOrLowerBet> gameService;
    ...
    void ReturnRtpFromOneMillionGamesPlayedInTwentyFourParallelThreads() throws ExecutionException, InterruptedException {
        ...
        for (int i = 0; i < NUMBER_OF_GAMES; i++) {
            Future<SinglePlayerSingleBetGameResult> future = executor.submit(() -> gameService.playGame(
                    createHigherOrLowerGameCommand(DEFAULT_BET, RANDOM.nextInt(1, 100))));
            futures.add(future);
        }
        double sum = 0;
